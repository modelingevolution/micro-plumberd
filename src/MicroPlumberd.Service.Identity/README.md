...
- A role dropped from the declaration is **left alone** â€” the seed is additive, it does not converge downwards.
...
- Uniqueness is enforced by the read models, not by the store. Two **different hosts** seeding the same empty store
  at the same instant can each create a role of the same name. Within one host â€” restarts, retries, concurrent
  attempts â€” the ensure steps are idempotent.
- The identity stores (`UserStore.AddToRoleAsync` / `RemoveFromRoleAsync` / `IsInRoleAsync`) assume the default
  upper-invariant `ILookupNormalizer`. A custom normalizer is **unsupported end-to-end** â€” the seed's own keys are
...
`ICaughtUpHandler.CaughtUp()` â€” not after a `Task.Delay`. Each write then waits until **its own write is visible**
...
host: the attempt logs at `Error`, and the runner retries with backoff `1, 2, 5, 10, 20, 30, 30â€¦` seconds until it
...
`State` live â€” Unhealthy naming the current step or the last error until the seed converged, Healthy afterwards.

## `HostOptions.BackgroundServiceExceptionBehavior` â€” state it
...
`IdentityInitializerService` never lets an exception escape `ExecuteAsync`, so it cannot trigger that on its own â€”
...
The library does not register it for you â€” neither does ASP.NET Core's own `AddIdentity`/`AddIdentityCore`. In a
...
`AddIdentityInitializer(o => â€¦)` and `IdentityInitializerOptions` keep their names and meaning. The call is now an
...
`ProjectionWaitTime` is no longer a delay â€” it is the per-attempt readiness bound.
